...
std::string timer_experiry(std::vector<PCB> &ready_queue, std::vector<PCB> &wait_queue, std::vector<PCB> &job_list, int current_time, PCB & running )
{
    std::string execution_status;
...
std::string update_running(std::vector<PCB> &ready_queue, std::vector<PCB> &wait_queue, std::vector<PCB> &job_list, int current_time, PCB & running )
{
    ...
    else if (running.io_freq > 0 && running.processing_time > 0 && running.processing_time % running.io_freq == 0)
    {
     ...
     FCFS(ready_queue);// the timer experiry will also trigger a schedular call, as the kicked process might be next
...
std::tuple<std::string /* add std::string for bonus mark */ > run_simulation(std::vector<PCB> list_processes) {
    ...
    while(!all_process_terminated(job_list) || job_list.empty()) { 
        ...
        //////////////////////////SCHEDULER//////////////////////////////
        FCFS(ready_queue); //scheudule next process 
        execution_status += timer_experiry(ready_queue,wait_queue,job_list, current_time,running);
        //already did the FCFS call in timer^
        execution_status += update_running(ready_queue,wait_queue,job_list, current_time,running);

        //start move to tick and consume CPU cycles 
